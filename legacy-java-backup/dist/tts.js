...
import { ttsHistoryStore } from './models.js';
...
async function responseFor(document) {
...
async function createTts(req, preview) {
...
ttsRouter.post('/tts/preview', optionalAuth, async (req, res, next) => {
    try {
        res.status(201).json({ data: await createTts(req, true) });
    }
    catch (error) {
        next(error);
    }
...
ttsRouter.post('/tts', optionalAuth, async (req, res, next) => {
    try {
        res.status(201).json({ data: await createTts(req, false) });
    }
    catch (error) {
        next(error);
    }
...
ttsRouter.get('/tts/:id', requireAuth, async (req, res, next) => {
    try {
        ...
        if (!doc)
            throw new AppError(404, 'TTS_NOT_FOUND', 'Không tìm thấy lịch sử TTS.');
